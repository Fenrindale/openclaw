...
describe("Databricks plugin", () => {
  ...
  describe("wrapStreamFn", () => {
    ...
    it("converts block-array toolResult content to string", async () => {
      const api = { registerProvider: vi.fn() } as any;
      await plugin.register(api);
      const wrapStreamFn = api.registerProvider.mock.calls[0][0].wrapStreamFn;
      ...
      const context = {
        messages: [
          ...
          {
            role: "assistant",
            content: [{ type: "toolCall", id: "call_1", name: "repl", arguments: "{}" }],
            stopReason: "toolUse",
          },
          {
            role: "toolResult",
      ...
      expect(toolMsg.content).toBe("line one line two");
    });
  });
  ...
  describe("non-interactive auth", () => {
    it("falls back to saved config baseUrl when CLI flag is omitted", async () => {
      const api = { registerProvider: vi.fn() } as any;
      await plugin.register(api);

      const providerReg = api.registerProvider.mock.calls[0][0];
      const authMethod = providerReg.auth[0];
      expect(authMethod.runNonInteractive).toBeDefined();

      // Simulate ctx with saved baseUrl in config but no CLI flag
      const ctx = {
        authChoice: "databricks-api-key",
        config: {
          models: {
            providers: {
              databricks: { baseUrl: "https://saved.cloud.databricks.com" },
            },
          },
        },
        baseConfig: {},
        opts: {
          // databricksBaseUrl is NOT provided (simulating omitted --databricks-base-url)
        },
      ...
      const result = await authMethod.runNonInteractive(ctx);
      // Should succeed (not return null) because saved baseUrl is available
      expect(result).not.toBeNull();
      // The resulting config should contain the saved baseUrl
      expect(result?.models?.providers?.databricks?.baseUrl).toBe(
        "https://saved.cloud.databricks.com",
      );
    });
  });
});
